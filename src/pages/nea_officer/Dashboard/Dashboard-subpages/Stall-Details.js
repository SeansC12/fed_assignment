    ...
    try {
        ...
        if (stallDoc.exists()) {
            ...
            if (stallData.ownerId) {
                const ownerDoc = await getDoc(doc(db, "vendor_list", stallData.ownerId));
                if (ownerDoc.exists()) updateOwnerUI(ownerDoc.data(), stallData.ownerId);
            }
        }
...
async function loadInspections() {
    ...
    const q = query(collection(db, "inspections"), where("stallId", "==", stallId), orderBy("timestamp", "desc"));
    const querySnapshot = await getDocs(q);

    // Stats Counters
    const counts = { A: 0, B: 0, C: 0, D: 0 };

    const rows = await Promise.all(querySnapshot.docs.map(async (inspectionDoc) => {
        ...
        if (insp.officerId) {
            const offQuery = query(collection(db, "nea-officer"), where("officerId", "==", insp.officerId));
...
function updateStallUI(data) {
    ...
    document.getElementById('stallAddress').textContent = `UNIT ${data.address || "---"}`;
    document.getElementById('displayStallId').textContent = stallId;
    document.getElementById('displayHawkerCentre').textContent = data.hawkerCentre || "N/A";
...
function updateOwnerUI(data, id) {
    ...
    document.getElementById('ownerInitial').textContent = (data.name || "?").charAt(0);
...
function renderTableRow(id, insp, officerName) {
    const date = insp.timestamp?.toDate ? insp.timestamp.toDate().toLocaleDateString('en-SG') : "N/A";
    return `
